...
def get_full_catalog(sample_catalog, rediscovered_catalog):
    ...
    """
    rediscovered_streams = [rs["stream"] for rs in rediscovered_catalog.get("streams", [])]
    # get matched (selected) streams
    matched_streams = ([s for s in sample_catalog["streams"] if s["stream"] in rediscovered_streams])
    ...
    for rediscovered_stream in rediscovered_catalog.get("streams", []):
        if rediscovered_stream["stream"] in list(selected_streams.keys()):
            ...
            for meta in rediscovered_stream["metadata"]:
                ...
                metadata[index]["metadata"]["replication-method"] = selected_streams.get(
...
def get_selected_streams(streams):
    ...
    for stream in streams:
        ...
        replication_method = metadata[0]['metadata']['replication-method'] or "FULL_TABLE"
        selected_streams.update({stream["stream"]: replication_method})
